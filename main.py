...
authorController = author.Author("NVR Notepad")
...
class NotePad:
    ...
    def __init__(self):
        ...
        if int(chkCurrentYear) > 2023:
            cpyrightYear = "2024 - " + str(chkCurrentYear)
        else:
            cpyrightYear = "2024"
    ...
    def _saveFile(self):
        ...
        if str(fileText.strip()) != "" and str(fileName.strip()) != "":
            ...
            try:
                ...
                with open(str(actualFile), writeAct, encoding="UTF-8") as tmp:
                    tmp.write(str(fileText))
                messagebox.showinfo("PS Notepad Alert", "File Saved Successfully.\nFilename :: " + str(actualFile) + "                         ")
                
            except:
                messagebox.showerror("PS Notepad Alert", "Something went wrong.                         ")
    ...
    def _closeNotePad(self):
        extBtn = messagebox.askquestion("PS Notepad Alert", "Are you sure you want to exit?                                                  ")
        if extBtn == 'yes':
